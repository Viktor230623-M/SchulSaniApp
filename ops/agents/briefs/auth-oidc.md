...
BEREICH: AREA: OIDC-Authentifizierung (artifacts/api-server/src/auth/providers/oidc.ts, registry.ts, app.ts). Prüfe: PKCE, State/Nonce, Redirect-URI-Validierung, Token-Handling (ID/access/refresh), Session-Cookie-Flags (HttpOnly, Secure, SameSite), Logout/Revocation, Multi-Tenant-School-Zuordnung.
