...
BEREICH: AREA: Passwort-Login & Reset (artifacts/api-server/src/auth/providers/local.ts, routes/auth.ts, paramedic-app Passwort-Flows). Prüfe: Hashing (Argon2?), Salt, Timing, Reset-Flow (Token-Entropie, Ablauf, Rate-Limit), Account-Lockout, E-Mail-Verifikation.
