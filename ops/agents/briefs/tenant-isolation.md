...
BEREICH: AREA: Multi-Tenant-Isolation (artifacts/api-server/src/middlewares/auth.ts, lib/schools.ts, schoolIdOf.ts, routes/ alle). Prüfe: Wird schoolId serverseitig aus der Session geholt statt vom Client? IDOR-Risiken, fehlende Tenant-Filter in Queries, Cross-Tenant-Zugriffe über IDs, Tests (tenantIsolation.integration.test.ts).
